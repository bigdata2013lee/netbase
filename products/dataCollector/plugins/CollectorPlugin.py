...
import products.netUtils.IpUtil as iputil
from products.dataCollector.plugins.DataMaps import ObjectMap, RelationshipMap
from products.netUtils.Utils import prepId as globalPrepId
import products.dataCollector.plugins.CommandPluginUtils as utils
ATTRIBUTES = (
    'id',
    'manageIp',
    'zMaxOIDPerRequest',
...
class CommandPlugin(CollectorPlugin):
    ...
    deviceProperties = CollectorPlugin.deviceProperties + (
        ...
        'netCommandUsername', 
        'netCommandPassword', 
        'netCommandLoginTries',
        'netCommandLoginTimeout', 
        'netCommandCommandTimeout',
        'netKeyPath', 
        'netCommandSearchPath', 
        'netCommandExistanceTest',
        'netSshConcurrentSessions'
        )

class IpmiPlugin(CollectorPlugin):
    """
    A CommandPlugin defines a command to be run on a remote device and
    a parsing methos to turn the commands output into a datamap.  A valid
    CommandPlugin must have class variable "command" defined and must implement
