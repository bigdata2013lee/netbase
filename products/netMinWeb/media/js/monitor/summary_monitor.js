(function($){
    ...
    m.summaryWidget = {
        ...
        _countInfo: function(info){
            ...
            self._data = {devices: f(info.devices), interfaces: f(info.interfaces), processes: f(info.processes), services: f(info.services)}
            
            if(!_xcount){_xcount = 1;}
            self._data.score = (self._data.devices.score +  self._data.interfaces.score +  self._data.processes.score +  self._data.services.score)/_xcount;
        ...
        reload:function(){
            var self = this;
            nb.rpc.monitorViews.c("getSummaryInfo")
