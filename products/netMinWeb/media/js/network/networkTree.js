(function($){
	var m = window.networkTree = new nb.xutils.Observer();
	var cookieName = "networkTreeViewStatusCache";
	var orgTypeName = "NetworkClass";
	var moTypeName = "Network";
	var remoteView = nb.rpc.treeViewApi;
	var remoteMethod = "getNetworkTreeDataSource2";
	var _panel_id="#network_class_tree";
			...
			var treeView = $(self._panel_id).kendoTreeView({
				...
				select: function(evt){
					...
					if(data._type == orgTypeName){
						m.fireEvent("selectOrgNode", data._id, data.uname, data.path);
					}
