(function($){
   ...
   m.reportContentWidget = {
    	...
    	_render:function(){
            ...
            var detailhtml = []; 
            detailhtmlwidget(self._data.cpu,"#detail_cpu_table","#detail_cpu_photo","#detail_cpu","cpu","利用率");
            detailhtmlwidget(self._data.mem,"#detail_mem_table","#detail_mem_photo","#detail_mem","mem","利用率");
            detailhtmlwidget(self._data.resTime,"#detail_resTime_table","#detail_resTime_photo","#detail_resTime","resTime","毫秒");
            detailhtmlwidget(self._data.availability,"#detail_availability_table","#detail_availability_photo","#detail_availability","availability","百分比");
            if (self._data.disk&&self._data.disk[0]) {
            	var table = [];
	            table.push("<table width='600px' class='tabChart' border='0' cellspacing='0' cellpadding='0'><tr><td class='tb01'>名称</td><td class='tb01'>分区大小</td><td class='tb01'>已用空间</td><td class='tb01'>利用率%</td></tr>");
	            var datavalue;
	            var classname;
            ...
            if (self._data.event&&self._data.event.length ==2) {
            	$("#detail_even_photo1").html('<img src="/chart_images/' + self._data.event[0].replace(imgsRealPath, "") + '" width="520px" />');
	       		$("#detail_even_photo2").html('<img src="/chart_images/' + self._data.event[1].replace(imgsRealPath, "") + '" width="220px" />');
            }
   ...
   function detailhtmlwidget(data,tableid,photoid,divid,name,title) {
	    if (data) {
	        if (data[0]) {
	            var table = [];
	            table.push("<table width='600px' class='tabChart' border='0' cellspacing='0' cellpadding='0'><tr><td class='tb01'>名称</td><td  class='tb01'>"+title+"%</td></tr>");
	            var datavalue;
	            var classname = "";
	            for (var i = 0; i < data[0].length; i++) {
	                ...
	                table.push("<tr><td  class='"+classname+"'>" + data[0][i].title + "</td><td  class='"+classname+"'>" + datavalue + "</td></tr>");
	            }
	            table.push("</table>");
	            $(tableid).html(table.join(""));
	        }
	        if (data.length == 2) {
	            $(photoid).html('<img src="/chart_images/' + data[1].replace(imgsRealPath, "") + '" width="600px" />');
	        }
	    } else {
	        $(divid).hide();
	    }
    ...
    m.editreportWidget = $.extend({}, editreportBaseWidget, {
        ...
        save: function(){
            ...
            success(function(msg){
        		...
        		window.location.href = "/report/content/"+rUid;
            });
        }
    });  
	...
	function GetData(RemotData){
		...
		_exportFormat = RemotData.exportFormat;
	...
	var outreport = function(){
		var html =[];
		html.push("<style>");
html.push("#Translation{ padding:30px;}");
html.push(".Trans_top,.Trans_top label{ font-size:24px; color:#f60; border-bottom:solid 2px #d4e3ea; padding-bottom:10px;}");
html.push(".Trans_top span{ width:104px; height:30px; line-height:30px; float:right;}");
html.push(".Trans_top span a{ width:104px; height:28px; line-height:28px; text-align:center; display:block; background-color:#ff8600; font-size:16px; color:#fff;border-radius:4px; border:solid 1px #f76e00; }");
html.push(".Trans_top span a:hover{ background-color:#1081db; border:solid 1px #0063b1;}");
html.push(".Trans_02{ font-size:14px; color:#333; border-bottom:solid 2px #d4e3ea; padding:10px 0 20px 0;}");
html.push(".dltrans01{ overflow:hidden; margin-top:10px;}");
html.push(".dltrans01 dt{ width:150px; float:left; text-align:right; font-size:15px; font-weight:bold; color:#0064b9;}");
html.push(".dltrans01 dd{ color:#555;}");
html.push(".dltrans02{ overflow:hidden; margin-top:15px;}");
html.push(".dltrans02 dt{ width:150px; float:left; text-align:right; font-size:15px; font-weight:bold; color:#0064b9; padding-top:15px;}");
html.push(".dltrans02 dd{   overflow:hidden;}");
html.push(".dltrans02 dd a{ float:left; display:block; padding:5px 10px; background-color:#2671c2;border-radius:4px; margin:10px 15px 0 0; color:#fff;}");
html.push(".dltrans02 dd a:hover{ background-color:#f60;}");
html.push(".dltrans03{ overflow:hidden; margin-top:10px;}");
html.push(".dltrans03 dt{ width:150px; float:left; text-align:right; font-size:15px; font-weight:bold; color:#0064b9; padding-top:15px;}");
html.push(".dltrans03 dd{ padding-top:12px; line-height:24px; color:#555; font-size:12px;   overflow:hidden;}");
html.push(".dltrans03 dd p{ color:#f30; padding:5px 0;}");

html.push(".Chart_01{ padding:20px 0; overflow:hidden;}");
html.push(".Chart_01_top{ line-height:24px; color:#555; overflow:hidden;}");
html.push(".Chart_01_top span{ font-size:15px; font-weight:bold;color:#0064b9; padding-right:10px; border-left: solid 4px #f60; padding-left:8px;}");
html.push(".Chart_tp{ padding:10px 20px; overflow:hidden;}");
html.push(".Chart_table{ padding:10px 20px;overflow:hidden;}");
html.push(".Chart_table .tabChart{  border-left:solid 1px #c7d3de; }");
html.push(".Chart_table .tabChart tr .tb01{ background-color:#216dad; border-right:solid 1px #157ed7; text-align:center;  line-height:24px; padding:4px 6px; color:#fff; font-size:14px; font-weight:bold;}");
html.push(".Chart_table .tabChart tr .tb02{ text-align:center; line-height:24px; padding:4px 6px; border-bottom:solid 1px #c7d3de; border-right:solid 1px #c7d3de;}");
html.push(".Chart_table .tabChart tr .tb03{ background-color:#f7f8f9; text-align:center; line-height:24px; padding:4px 6px; border-bottom:solid 1px #c7d3de; border-right:solid 1px #c7d3de;}");
html.push("</style>");
		html.push($("#reportContentWidget").html());
		var ohtml = html.join("");
		ohtml = ohtml.replace('<a class="outreport" href="#">导出报表</a>',"");
		nb.rpc.reportApi.c("exportReport", {htmlContent:ohtml,etype:_exportFormat}).success(function(data){
			//data.True = "http://netbase.wanjee.cn/media/upload_docs/Netbase_Zenpack模板整理信息.doc";
			if(data.False){
				nb.AlertTip.warn(data.False);
			}
			else if(data.True){
				 var msg="<a href='/chart_images/"+data.True.replace(imgsRealPath,'')+"'>报表文件导出成功，<font color='red'>请点击这里下载报表</font>.</a>";
