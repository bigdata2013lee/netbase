//省市数据字典对象定义
window.nb_areas = {
    "直辖市":["南京","上海","重庆","天津","青岛","大连","沈阳","哈尔滨","西安","北京"],
...
//省市对象全局变量的定义
window.nb_areasSearch = function(text, existLabes){
    ...
    var _fileds={}
    for(pLabel in window.nb_areas){
        for(var i=0; i < window.nb_areas[pLabel].length; i++){
            var item = window.nb_areas[pLabel][i];
            if(inArray(item, existLabes)){continue;}
            if(item.toLowerCase().search(text.toLowerCase())>=0){
