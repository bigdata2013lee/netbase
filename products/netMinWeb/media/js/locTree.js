(function($){
	var m = window.locTree = new nb.xutils.Observer();
	var cookieName = "LocationTreeViewStatusCache";
	var orgTypeName = "Location";
	var moTypeName = "Device";
	var remoteView = nb.rpc.treeViewApi;
	var remoteMethod = "getLocationDevTreeDataSource";
	var _panel_id="#location_tree";
	...
	m.treeWidget = {
			...
			var treeView = $(self._panel_id).kendoTreeView({
				...
				select: function(evt){
					...
					if(data._type == orgTypeName){
						m.fireEvent("selectOrgNode", data._id, data.uname, data.path, data.title);
					}
        ...
        getSelectNodeData:function(){
        	var self = this;
        	var data_uid = $(self.getTreeView().select()).attr("data-uid");
        	return self.ds().getByUid(data_uid);
        },
		reload: function(){
			var self = this;
			/** 统计mo对象数*/
