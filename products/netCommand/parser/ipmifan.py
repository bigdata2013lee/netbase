...
class ipmifan(CommandParser):
    """
    功能:ipmi风扇命令行输出结果的解析器
    作者:wl
    时间:2013-1-7
    """
    def processResults(self, cmd, result):
        ...
        if exitCode==0:
            output = output.split('\n')[0].strip()
            fanValues= output.split(":")
            status=fanValues[3].strip()[1:-1]
            if status=="OK":
                severity=0
                msg = '%s状态正常!'%cmd.title
            else:
                severity=5
                msg = '%s状态异常!'%cmd.title
            letRpm=fanValues[2].strip().split(" ",1)[0]           
            rs=dict(status=status,letRpm=letRpm)
