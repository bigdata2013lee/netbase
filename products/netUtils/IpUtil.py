...
from products.netUtils.ipaddr import IPNetwork,IPAddress
...
def getAllSegmentIps(network):
    ...
    if (netmask==net.max_prefixlen): return [network]
    ipnumb = long(int(net))
    maxip = math.pow(2,net.max_prefixlen - netmask)
    start = int(ipnumb+1)
    end = int(ipnumb+maxip-1)
    def toIPAddress(ip):
        return str(IPAddress(ip))
    return map(toIPAddress,range(start,end))

def getRangeIps(startIp,endIp):
    """
        得到范围内的所有IP
    """
    start = int(IPNetwork(startIp))
    end = int(IPNetwork(endIp))
    def toIPAddress(ip):
        return str(IPAddress(ip))
    return map(toIPAddress,range(start,end))

if __name__=="__main__":
    getRangeIps("192.168.2.1","192.168.3.6")
